 ...
 * Signature: SmartScore rings + LION/MONKEY/DOG classification + SLA countdown + Priya stage
 * Views: List view (default) + Kanban
 */
...
import {
  ...
  Calendar, ChevronRight, ArrowUpDown, Filter, Plus,
} from 'lucide-react'
import {
  SmartScoreRing, ClassBadge, SlaTimer, PriyaStagePill, SourcePill,
  GlassCard, MOCK_LEADS, PRIYA_STAGES,
  type Lead, type Classification,
} from './AgenticShared'

// ─── Kanban stages ────────────────────────────────────────────────────────────

const KANBAN_STAGES = ['Lead Capture', 'New Lead', 'Contacted', 'Qualified', 'Site Visit', 'Negotiation', 'Booked', 'Closed Won']

// ─── Score breakdown bar ──────────────────────────────────────────────────────

function ScoreBreakdown({ breakdown }: { breakdown: NonNullable<Lead['scoreBreakdown']> }) {
  const bars = [
    { label: 'Budget',    val: breakdown.budget,    max: 30, color: 'bg-amber-500' },
    { label: 'Timeline',  val: breakdown.timeline,  max: 30, color: 'bg-yellow-500' },
    { label: 'Behavioral',val: breakdown.behavioral,max: 25, color: 'bg-blue-500'  },
    { label: 'Intent',    val: breakdown.intent,    max: 15, color: 'bg-emerald-500'},
  ]
  return (
...
function LeadDetailPanel({ lead, onClose }: { lead: Lead; onClose: () => void }) {
  const [activeTab, setActiveTab] = useState<'score' | 'timeline' | 'actions'>('score')
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-5 py-3 border-b border-white/[0.06]">
        ...
        <span className="ml-auto text-[11px] text-zinc-500">{lead.pipelineStage}</span>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-white/[0.06]">
        {(['score', 'timeline', 'actions'] as const).map(t => (
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {activeTab === 'score' && (
          <div className="space-y-5">
            ...
            {lead.scoreBreakdown && <ScoreBreakdown breakdown={lead.scoreBreakdown} />}
            <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.07]">
              <h4 className="text-[11px] font-semibold text-zinc-400 uppercase mb-3">AI Priya Progress</h4>
              <div className="space-y-2">
                {PRIYA_STAGES.map((stage, i) => {
                  const idx = PRIYA_STAGES.indexOf(lead.priyaStage)
                  const done = i <= idx
                  const current = i === idx
                  return (
        ...
        {activeTab === 'timeline' && (
          <div className="space-y-3">
            {[
              { time: 'Just now', event: `${lead.priyaStage.replace(/_/g, ' ')} via WhatsApp Priya`, color: 'bg-amber-400' },
              { time: '2h ago', event: 'Lead qualified — SmartScore assigned', color: 'bg-emerald-400' },
              { time: '3h ago', event: `Assigned to ${lead.assignedTo}`, color: 'bg-blue-400' },
              { time: '3h ago', event: `Lead captured from ${lead.source}`, color: 'bg-zinc-500' },
            ].map((e, i) => (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div className={`w-2 h-2 rounded-full ${e.color} flex-shrink-0 mt-1`} />
                  {i < 3 && <div className="w-px flex-1 bg-white/[0.06] mt-1" />}
                </div>
                <div className="pb-3">
                  <p className="text-xs text-zinc-200">{e.event}</p>
          ...
          <div className="space-y-3">
            <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.07] space-y-2">
              ...
              {[
                ...
                { label: 'Assigned', value: lead.assignedTo },
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-zinc-500">{lead.budget}</p>
        </div>
      </div>
      <div className="flex items-center gap-1.5 flex-wrap">
        <ClassBadge classification={lead.classification} showEmoji={false} />
        <SourcePill source={lead.source} />
      </div>
      <div className="flex items-center justify-between">
        <PriyaStagePill stage={lead.priyaStage} />
...
export function LeadsPipelineSection() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== 'ALL') leads = leads.filter(l => l.classification === filter)
    if (search.trim()) {
      const q = search.toLowerCase()
      leads = leads.filter(l => l.name.toLowerCase().includes(q) || l.phone.includes(q))
    }
    if (sortBy === 'score') leads.sort((a, b) => b.smartScore - a.smartScore)
    if (sortBy === 'sla')   leads.sort((a, b) => a.slaDeadline.getTime() - b.slaDeadline.getTime())
    if (sortBy === 'name')  leads.sort((a, b) => a.name.localeCompare(b.name))
    return leads
  }, [filter, search, sortBy])

  const counts = {
    ALL: MOCK_LEADS.length,
    LION: MOCK_LEADS.filter(l => l.classification === 'LION').length,
    MONKEY: MOCK_LEADS.filter(l => l.classification === 'MONKEY').length,
    DOG: MOCK_LEADS.filter(l => l.classification === 'DOG').length,
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500">{MOCK_LEADS.length} total · SmartScore ranked</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center rounded-xl bg-white/[0.04] border border-white/[0.07] p-1 gap-1">
          {(['ALL', 'LION', 'MONKEY', 'DOG'] as const).map(cls => {
            const emoji = { ALL: '📋', LION: '🦁', MONKEY: '🐒', DOG: '🐕' }[cls]
            return (
              <button
                key={cls}
                onClick={() => setFilter(cls)}
                className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all duration-200 ${filter === cls ? 'bg-amber-500/20 border border-amber-500/30 text-amber-400' : 'text-zinc-500 hover:text-zinc-300'}`}
              >
                {emoji} {cls} {cls !== 'ALL' && <span className="opacity-60">({counts[cls]})</span>}
              </button>
            )
          })}
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Lions 🦁', value: String(filtered.filter(l => l.classification === 'LION').length), color: 'text-amber-400' },
          { label: 'Breached SLA', value: String(filtered.filter(l => l.slaDeadline.getTime() < Date.now()).length), color: 'text-red-400' },
          { label: 'In Booking', value: String(filtered.filter(l => l.priyaStage === 'BOOKING').length), color: 'text-emerald-400' },
        ].map(s => (
      ...
      {view === 'list' && (
        <GlassCard className="overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[48px_1fr_100px_80px_100px_100px] gap-4 px-5 py-3 border-b border-white/[0.06] text-[10px] font-semibold text-zinc-600 uppercase tracking-wider">
            <span>Score</span>
            <span>Lead</span>
            <span>Class</span>
            <span>Stage</span>
            <span className="hidden sm:block">Priya</span>
            <span>SLA</span>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {filtered.map(lead => (
              <motion.div
                ...
                className="grid grid-cols-[48px_1fr_100px_80px_100px_100px] gap-4 px-5 py-3.5 hover:bg-white/[0.03] cursor-pointer transition-colors items-center"
              >
                ...
                <ClassBadge classification={lead.classification} />
                <span className="text-[11px] text-zinc-400 truncate">{lead.pipelineStage}</span>
                <div className="hidden sm:block"><PriyaStagePill stage={lead.priyaStage} /></div>
                <SlaTimer deadline={lead.slaDeadline} classification={lead.classification} />
              </motion.div>
            ))}
          </div>
          ...
          <div className="flex gap-3 min-w-max">
            {KANBAN_STAGES.map(stage => {
              const stageLeads = filtered.filter(l => {
                if (stage === 'Site Visit') return l.pipelineStage.includes('Visit')
                return l.pipelineStage === stage
              })
              return (
                <div key={stage} className="w-56 flex-shrink-0">
                  <div className="flex items-center justify-between mb-2 px-1">
                    <span className="text-[11px] font-semibold text-zinc-400">{stage}</span>
