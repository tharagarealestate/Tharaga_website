...
export default function AdminVerifyPage(){
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-8">
      <h1 className="text-2xl font-bold text-deepBlue mb-2">Admin · Verify properties</h1>
      ...
      <div className="space-y-2">
        {rows.map(r => (
          <div key={r.id} className="rounded-lg border border-deepBlue/10 bg-white p-3 flex items-center justify-between">
            <div>
              <div className="font-semibold text-deepBlue">{r.title}</div>
              <div className="text-xs text-deepBlue/70">{[r.locality, r.city].filter(Boolean).join(', ')}</div>
            </div>
            <button onClick={()=>verify(r.id)} className="rounded-lg bg-gold text-deepBlue px-3 py-1 text-sm border border-transparent hover:brightness-105">Verify</button>
          </div>
        ))}
        {!rows.length && !busy && <div className="text-sm text-deepBlue/70">No pending properties.</div>}
      </div>
    </main>
  )
}
