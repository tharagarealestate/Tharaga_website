...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[Pipeline API] Authorization header:', authHeader ? 'present' : 'missing')
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      console.log('[Pipeline API] Verifying token from Authorization header...')
      ...
      if (!tokenError && tokenUser) {
        user = tokenUser
        console.log('[Pipeline API] Authenticated via token:', tokenUser.email)
        supabase = tokenClient as any
      } else {
        authError = tokenError
        console.error('[Pipeline API] Token verification failed:', tokenError?.message)
      }
    } else {
