...
export const Crane: React.FC<{ color?: string; className?: string }> = ({ color = '#1A73E8', className }) => {
  return (
    <motion.svg
      ...
      viewBox="0 0 300 300"
      className={className}
      initial={{ rotate: -6, y: -18, opacity: 0 }}
    ...
    >
      ...
      <g stroke={color} strokeWidth="3" fill="none" strokeLinecap="round">
        <motion.path d="M200 160 l30 40" animate={{ rotate: [0, 4, 0, -4, 0] }} transition={{ repeat: Infinity, duration: 3 }} />
        <motion.circle cx="230" cy="200" r="14" fill={color} initial={{ scale: 0 }} animate={{ scale: [0.9, 1, 0.9] }} transition={{ repeat: Infinity, duration: 1.8, delay: 0.2 }} />
      </g>
    </motion.svg>
  )
}
