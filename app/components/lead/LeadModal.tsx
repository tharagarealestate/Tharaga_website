...
export function LeadModal({ propertyId, open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl bg-white border border-deepBlue/10 p-4 shadow-subtle" onClick={(e)=>e.stopPropagation()}>
        <div className="flex items-center justify-between mb-2">
          <div className="font-semibold text-deepBlue">Request details</div>
          <button onClick={onClose} aria-label="Close">✕</button>
        </div>
        {ok && <div className="mb-2 text-sm text-deepBlue/80">{ok}</div>}
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button disabled={busy} className="rounded-lg bg-deepBlue text-white px-4 py-2">Send</button>
          </div>
        </form>
      </div>
    </div>
