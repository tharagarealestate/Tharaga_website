...
export class ReraVerificationService {
  ...
  async verifyRera(input: VerifyReraInput): Promise<ReraVerificationResult> {
    ...
    if (this.partnerApiKey && this.partnerApiUrl) {
  ...
  private async verifyViaPartner(
    ...
    try {
      const response = await fetch(`${this.partnerApiUrl}/verify`, {
        method: 'POST',
        headers: {
  ...
  private async queueManualVerification(
    ...
    await this.supabase.from('rera_compliance_alerts').insert({
...
export const reraVerificationService = new ReraVerificationService();
