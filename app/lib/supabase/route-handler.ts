 ...
 * 
 * CRITICAL: Also checks Authorization header for token-based auth
 * (Supabase client-side uses localStorage, not cookies)
 */
export function createClientFromRequest(request: NextRequest) {
  const response = NextResponse.next()
  
  // Check for Authorization header (token from localStorage)
  ...
  return {
    supabase,
    response,
    token // Expose token for direct verification if needed
  }
}
