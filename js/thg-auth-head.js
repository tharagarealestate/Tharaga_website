...
;(function(){
  ...
  try {
    if (!document.querySelector('script[data-thg-auth-gate]')){
      ...
      try { window.__thgAuthGateReady = false; } catch(_){}
      s.onload = function(){ try { window.__thgAuthGateReady = true; } catch(_){} };
...
`; const style = document.createElement('style'); style.id = 'thg-auth-styles'; style.textContent = css; document.head.appendChild(style); }
    ...
    function bindUI(ui){
      ui.btn.addEventListener('click', function(e){ e.preventDefault(); if (state.user && state.user.email){ toggleMenu(ui); } else { try { if (window.__thgAuthGateReady === true && window.authGate && typeof window.authGate.openLoginModal === 'function'){ window.authGate.openLoginModal({ next: location.pathname + location.search }); return; } } catch(_){ } openAuthModal(ui, { next: location.pathname + location.search }); } });
