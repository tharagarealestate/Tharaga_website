...
function setLocalityAllSelected(){
  const el = document.querySelector('#locality'); if (!el) return;
  Array.from(el.options).forEach(o => o.selected = (o.value === 'All'));
}
function hydrateCityOptions(){
  const sel = document.querySelector('#city'); if (!sel) return;
  ...
  sel.addEventListener('change', ()=>{ const selectedCities = Array.from(sel.selectedOptions).map(o=>o.value); hydrateLocalityOptions(selectedCities); PAGE=1; apply(); });
}
function hydrateLocalityOptions(selectedCities){
  const localitySelect = document.querySelector('#locality'); if (!localitySelect) return;
...
function collectFilters(){
  ...
  const mode = activePill && activePill.dataset.type?.toLowerCase() !== 'all' ? (activePill.dataset.type || '').toLowerCase() : '';
  const cityEl = document.querySelector('#city');
  const citySel = cityEl ? Array.from(cityEl.selectedOptions).map(o=>o.value) : [];
  const localitySel = document.querySelector('#locality') ? Array.from(document.querySelector('#locality').selectedOptions).map(o=>o.value) : [];
  ...
  activeFilterBadges({ q, city: citySel, locality: localitySel, price:`${minP||''}-${maxP||''}`, type: ptype, bhk, furnished, facing, area:`${minA||''}-${maxA||''}`, amenity, metro: wantMetro ? `≤ ${maxWalk} min` : '' });
...
function apply(){
  ...
  let filtered = ALL.filter(p=>{
    if (F.mode) { const pc = String(p.category || p.propertyCategory || '').toLowerCase(); if (pc !== F.mode) return false; }
    if (F.q) { const t = (p.title+' '+p.project+' '+p.city+' '+p.locality+' '+(p.address||'')+' '+(p.summary||'')).toLowerCase(); const toks = F.q.toLowerCase().split(/\s+/).filter(Boolean); if (!toks.every(tok => t.includes(tok))) return false; }
  ...
  const pages = Math.max(1, Math.ceil(total / PAGE_SIZE)); PAGE = Math.min(PAGE, pages);
  const start = (PAGE-1)*PAGE_SIZE; const slice = filtered.slice(start, start+PAGE_SIZE);

  const res = document.querySelector('#results');
  if (res) res.innerHTML = slice.map(({p,s})=> App.cardHTML(p, s)).join('') || `<div class="empty" style="padding:28px;text-align:center;color:var(--muted)">No properties found</div>`;

  const pager = document.querySelector('#pager');
  if (pager) {
    pager.innerHTML = Array.from({length: pages}, (_,i)=>{ const n = i+1; const cls = n===PAGE ? 'page active' : 'page'; return `<button class="${cls}" data-page="${n}">${n}</button>`; }).join('');
  }
}
...
function wireUI(){
  ...
  ['#minPrice','#maxPrice','#minArea','#maxArea','#amenity','#q'].forEach(sel=> document.querySelector(sel)?.addEventListener('input', debApply));
  ['#sort','#ptype','#bhk','#furnished','#facing','#city','#locality','#wantMetro','#maxWalk'].forEach(sel=> document.querySelector(sel)?.addEventListener('change', ()=>{ PAGE=1; apply(); }));
  ...
  document.querySelectorAll('.pill').forEach(btn=>{ btn.addEventListener('click', ()=>{ document.querySelectorAll('.pill').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); PAGE=1; apply(); }); });
...
async function init(){
  try{
    await App.initConfig(); // <-- important: loads config (netlify function or window.CONFIG) and init supabase
